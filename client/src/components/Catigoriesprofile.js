...
formData.append('userId',this.props.todos._id);
...
fetch("http://localhost:8000/catgories", option)
  ...
  .then((res) => {
    if (res) {
      console.log(res)
    this.setState({
      messagedisplay: " Success! Succefully your Product HasBeen Published"
    })
  ...
  autoFocus: true
    ...
    <select ref="typeselected" className="form-control" required>
            ...
            <option value="TextileFabrics">Textile&Fabrics</option>
            ...
            <option value="CosmeticsProducts">CosmeticsProducts</option>
...
const mapStateToProps = state => ({ todos: state.loginreducers })
  ...
  const showloginname = connect(mapStateToProps)(Profile);
